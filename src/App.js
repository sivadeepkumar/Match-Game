...
class App extends Component {
  ...
  playAgain = async () => {
    this.setState({score: 0, sixtySecond: 60, popUp: false})
  ...
  tick = () => {
    const {sixtySecond} = this.state
    if (sixtySecond === 0) {
      this.componentWillUnmount()
    } else {
      this.setState(prevState => ({sixtySecond: prevState.sixtySecond - 1}))
    }
  }
    ...
    return (
      <div>
        <div className="main-container">
          <NavBar score={score} sixtySecond={sixtySecond} />
          {popUp && (
